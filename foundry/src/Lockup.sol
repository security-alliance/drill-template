...
/*
    ...
    can be set up. With this in mind, some relevant design choices include:
        ...
        upgradeable.
*/
contract Lockup is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    ***************************************************************************/
    ...
    function lockup(IERC20 _token, address _recipient, uint256 _amount) external whenNotPaused {
        currentId += 1;
    ...
    function claim(uint256[] calldata _ids) external whenNotPaused {
        for (uint256 i; i < _ids.length; ++i) {
            ...
            if (i != 0) require(_ids[i - 1] < _ids[i], "input ids are not increasing");
            require(l.startTime != 0, "lockup doesn't exist");
            require(l.startTime + LOCKUP_PERIOD < block.timestamp, "lockup not complete");
            
            l.token.transfer(l.recipient, l.amount);
